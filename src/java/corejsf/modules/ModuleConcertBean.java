...
public class ModuleConcertBean implements Serializable {
    ...
    public String[] getTableFields () {
		...
		arr[0] = "Страна";
		arr[1] = "Место проведения";
		arr[2] = "Дата проведения";
		return arr;
    }
    ...
    public Item[] getTableItems () {
		try {
			...
			while (result.next ()) {
				...
				item.setPublicValue (0, result.getString ("country"));
				item.setPublicValue (1, result.getString ("place"));
				item.setPublicValue (2, Utils.toNormDate (result.getString ("concert_date")));
				item.setEditValue (0, result.getString ("country"));
				item.setEditValue (1, result.getString ("place"));
				item.setEditValue (2, Utils.toNormDate (result.getString ("concert_date")));
